...
/*  jzp_printf("ADDSEG: addr %.4X len %.4X set %.2X clr %.2X\n", addr, len, set_attr, clr_attr);*/
    ...
    /* -------------------------------------------------------------------- */
    for (uint_32 sa = addr; sa < addr + len; sa += 256)
    {
        const uint_32 sp = sa >> 8;
...
/*      jzp_printf("ATTR: set %.2X clr %.2X on %.2X00\n", set_attr,clr_attr,sp);*/
        ...
        const uint_32 ea = (sa + 255 > addr + len ? addr + len : sa + 255) - 1;
        const uint_32 ep = ea >> 8;
                      ...
                      const int silent)
{
    ...
    uint_8 *const rom_img = CALLOC(uint_8, len);
    if (!rom_img)
    {
        if (silent) return -1;
        perror("malloc()");

...
/* ======================================================================== */
LOCAL INLINE uint_32 icart_calc_bs(icart_t *const ic, uint_32 const addr)
{
...
/* ======================================================================== */
uint_32 icart_rd_NULL(periph_t *const per, periph_t *const ign, 
                      const uint_32 addr, const uint_32 data)
{
    UNUSED(per); UNUSED(ign); UNUSED(addr); UNUSED(data);
    return ~0U;
}
uint_32 icart_rd_f(periph_t *const per, periph_t *const ign,
                   const uint_32 addr, const uint_32 data)
{
    icart_t *const ic = (icart_t *)(per->parent);
    UNUSED(ign); UNUSED(data);
    return ic->rom.image[addr];
}
uint_32 icart_rd_fn(periph_t *const per, periph_t *const ign,
                    const uint_32 addr, const uint_32 data)
{
    icart_t *const ic = (icart_t *)(per->parent);
    UNUSED(ign); UNUSED(data);
...
uint_32 icart_rd_b(periph_t *const per, periph_t *const ign,
                   const uint_32 addr, const uint_32 data)
{
    icart_t *const ic = (icart_t *)(per->parent);
    UNUSED(ign); UNUSED(data);
...
uint_32 icart_rd_bn(periph_t *const per, periph_t *const ign,
                    const uint_32 addr, const uint_32 data)
{
    icart_t *const ic = (icart_t *)(per->parent);
    UNUSED(ign); UNUSED(data);
...
/* ======================================================================== */
uint_32 icart_rd_bs(periph_t *const per, periph_t *const ign,
                    const uint_32 addr, const uint_32 data)
{
    UNUSED(per); UNUSED(ign); UNUSED(addr); UNUSED(data);
    return ~0U;
...
void icart_wr_bs(periph_t *const per, periph_t *const ign,
                 const uint_32 w_addr, const uint_32 data)
{
    icart_t *const ic = (icart_t *)(per->parent);
    ...
    const uint_32 i_addr = ((w_addr & 0xF) << 12) | ((w_addr & 0x10) << 7);

    const uint_32 b_idx =  i_addr >> 13;
    const uint_32 b_shf = (i_addr >> 8) & 31;
    if (((ic->rom.dobanksw[b_idx] >> b_shf) & 1) == 0)
        return;

    /* Convert the write data + Intellivision address to a bankswitch entry. */
    const uint_32 t_idx = i_addr >> 11;
    const uint_32 bs    = ((data << 8) - (i_addr & ~0x0700)) & 0xFFFF;

    ic->bs_tbl[t_idx] = bs;

    if (ic->cache_bs && ((ic->rom.readable[b_idx] >> b_shf) & 1) == 1)
    {
        cp1600_invalidate(ic->cpu, i_addr, i_addr + 0x00FF);
    }
}
...
/* ======================================================================== */
LOCAL void icart_dtor(periph_p const p)
{
    icart_t *const icart = (icart_t *)p;
...
)
{
    ...
    if (randomize)
        for (int i = 0x00; i < 0x1F; i++)
            icart_wr_bs((periph_t*)ic, NULL, i, rand_jz());
...
(
    icart_t     *const ic,
    periph_bus_p const bus,
    cp1600_t    *const cpu,
    uint_32      const cache_flags
)
{
    int         attr, p_attr;
    char        buf[17];

...
LOCAL void mem_wr_p16r(periph_t *per, periph_t *ign, uint_32 addr, uint_32 data)
{
    mem_t *mem = (mem_t*)per;
    int range, page;
...
LOCAL void mem_wr_p16w(periph_t *per, periph_t *ign, uint_32 addr, uint_32 data)
{
    mem_t *mem = (mem_t*)per;
    int range, page;
    ...
    if ( mem->page == mem->page_sel )
    {
        mem->image[addr] = data & mem->data_mask;
        if (mem->cpu)
            cp1600_invalidate((cp1600_t*)mem->cpu, addr, addr);
    }
}
...
)
{
    ...
    /*  Set up the mem-specific fields.                                     */
    /* -------------------------------------------------------------------- */
    mem->image      = CALLOC(uint_16, 1u << size);
    mem->img_length = 1u << size;
    mem->data_mask  = ~((~0U) << width);
    mem->chk_jlp    = 0;

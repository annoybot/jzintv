...
LOCAL uint_32 snd_tick(periph_p periph, uint_32 len);
...
/*  WAV header.                                                             */
/* ======================================================================== */
LOCAL const uint_8 snd_wav_hdr[44] =
{
...
LOCAL uint_32 snd_tick(periph_p periph, uint_32 len)
{
    ...
    int min_num_dirty;
    int i, j, k, mix;
    int try_drop = 0, did_drop = 0, dly_drop = 0, drop_all = 0;
    sint_16 *clean;
    uint_64 new_now;
...
one_source:
        ...
        {
            fwrite(clean, sizeof(sint_16), snd->buf_size,
                    snd->raw_file);
            snd->raw_start = 1;
        }
 ...
 * ============================================================================
 */
LOCAL void snd_fill(void *udata, uint_8 *stream, int len)
{
...
)
{
    ...
    src->buf   = CALLOC(sint_16,   snd->buf_size * src->num_clean);
    src->clean = CALLOC(sint_16 *, src->num_clean);
    src->dirty = CALLOC(sint_16 *, src->num_clean);
 ...
 * ============================================================================
 */
LOCAL const char *snd_fmt_name(uint_16 fmt)
{
...
LOCAL void snd_dtor(periph_p p);
             ...
             struct avi_writer_t *const avi, int pal_mode, double time_scale)
{
    ...
    snd->mixbuf.buf   = CALLOC(sint_16,   snd->buf_size * snd->mixbuf.num_clean);
    snd->mixbuf.clean = CALLOC(sint_16 *, snd->mixbuf.num_clean);
    snd->mixbuf.dirty = CALLOC(sint_16 *, snd->mixbuf.num_clean);

    if (mixbuf) free(mixbuf);
    mixbuf            = CALLOC(sint_32,   snd->buf_size);
...
{
    snd_t     *snd = (snd_t *)p;
    snd_pvt_t *pvt = (snd_pvt_t *)snd->pvt;
    int i;

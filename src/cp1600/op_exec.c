...
LOCAL uint_16 ext_addr_read(const instr_t *instr, cp1600_t *cp1600)
{
    extern int lto_isa_enabled;
    const int amode = instr->opcode.decoder.amode;
    const uint_16 reg0 = instr->opcode.decoder.reg0;
    const uint_16 imm0 = instr->opcode.decoder.imm0;
    const uint_16 imm1 = instr->opcode.decoder.imm1;
    const uint_16 xreg = instr->opcode.decoder.xreg0;
    const uint_16 base = cp1600->xr[xreg];
    const uint_16 addr = base + imm0;
    ...
    if (!amode)
    {
        ...
        if (reg0 == 7 && imm1 == 7)
        {
            const uint_16 dest = (cp1600->r[7] - 1) + imm0;
            const uint_16 delt = cp1600->r[7] ^ dest;
...
LOCAL void ext_addr_write(const instr_t *instr, cp1600_t *cp1600, uint_16 data)
{
    extern int lto_isa_enabled;
    const int amode = instr->opcode.decoder.amode;
    const uint_16 imm0 = instr->opcode.decoder.imm0;
...
#define SDBD_DIRECT(data,cp1600,cycles) \
    if (cp1600->D)                                                  \
    {                                                               \
        uint_16 sdbd_data = CP1600_RD(cp1600, cp1600->r[7]);        \
        cp1600->r[7]++;                                             \
...
static uint_16 handle_atomic(const instr_t *instr, cp1600_t *cp1600,
                             uint_16 addr, uint_16 data)
{

...
#if defined(linux) || defined(termux)
...
static const int joy_dir_map[MAX_JOY][MAX_STICKS] =
{
...
/*jzp_printf("iters=%d axis=%d then=%f now=%f\n", iters, a, then*16, now*16);*/
    ...
    *ev_updn = stick->disc_dir == -1 ? UP : DOWN;

    if (best_dotp < stick->rels_thresh && stick->disc_dir != -1)
    {
        *ev_updn = UP;
        *ev_num  = joy_dir_map[i][st] + stick->disc_dir;
        stick->disc_dir = -1;
        return;
    }
                                    ...
                                    int *ex_updn, uint_32 *ex_num)
{
    ...
    if (*ev_num == EVENT_IGNORE || *ex_num != EVENT_IGNORE)
        may_combo = 0;

...
{
    uint_16     buffer[AVI_AUDIO_BUF];
    uint_16     *buffer_read;
    uint_16     *buffer_write;
...
typedef struct avi_video_t
{
    avi_frame_t frame[2];
    uint_8      toggle;
    uint_8      palette[256][3];
    uint_8      encode_buf[AVI_VIDEO_BUF_BYTES];
    uint_32     encode_buf_wr, encode_buf_rd;
    uint_32     encode_len[AVI_VIDEO_BUF_FRAMES];
    uint_32     encode_len_wr, encode_len_rd;
    uint_32     advance_frames_remaining;
    double      time_remainder;
...
{
    const uint_64 curr = (sint_64)info->total_frames * info->usec_per_frame;
    const uint_64 next = curr + info->usec_per_frame;
    const uint_32 curr_samples = (curr * info->audio_rate) / 1000000;
    const uint_32 next_samples = (next * info->audio_rate) / 1000000;
...
)
{
    ...
    if (!video->advance_frames_remaining)
    {
        ...
        uint_8 *const encoded_video = video->encode_buf + video->encode_buf_rd;
        encoded_video_size =
    ...
    const uint_32 encoded_audio_size = audio_frame_size * sizeof(sint_16);
    if (video->advance_frames_remaining || !flush)
    {
        riff_start_element(riff, "01wb");
        uint_32 audio_remain = audio_frame_size;
        if (audio->buffer_read + audio_remain > audio->buffer + AVI_AUDIO_BUF)
        {
            const uint_32 c = (audio->buffer + AVI_AUDIO_BUF) -
                                audio->buffer_read;
            if (c)
                fwrite(audio->buffer_read, 1, c * sizeof(sint_16), riff->file);
            audio->buffer_read = audio->buffer;
            audio->buffer_total_read += c;
            audio_remain -= c;
        }
        fwrite(audio->buffer_read, sizeof(sint_16), audio_remain, riff->file);
    ...
    const uint_32 container_size = riff_close_structure(riff);
...
)
{
    ...
    if (audio->buffer_write + num_samples > audio->buffer + AVI_AUDIO_BUF) {
        ...
        if (wrap)
            memcpy(audio->buffer_write, audio_data, wrap * sizeof(sint_16));
        audio->buffer_write = audio->buffer;
    }

    memcpy(audio->buffer_write, audio_data + wrap,
           (num_samples - wrap) * sizeof(sint_16));
...
LOCAL INLINE uint_8 *send_block_delta(const avi_frame_t *const RESTRICT prev,
                                      const avi_frame_t *const RESTRICT curr,
                                      const int px, const int py,
                                      const int cx, const int cy,
                                      uint_8 *const RESTRICT out)
{
    int x, y, cnt = 0;
...
LOCAL INLINE uint_8 *send_frame
(
    const uint_8 *const palette,
    void *const data, const int data_size,
    uint_8 *const output, const int output_avail,
...
{
    if (avi->pvt)
        memcpy((void*)avi->pvt->video.palette, palette, len * 3);
}
...
)
{
    ...
    {
        const uint_8 *RESTRICT vid_row_i = image + y * (INPUT_DIM_X/2);
        uint_8 *RESTRICT vid_row_o =
    ...
    if (key_frame)
    {
        ...
        *output++ = BLOCK_HEIGHT;   /* Block height */
        ...
        const uint_32 remain = AVI_VIDEO_ENCODE_BYTES - (output - base_output);
    ...
    const uint_32 length = output - base_output;

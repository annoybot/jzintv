...
uint_32 pad_tk_event(periph_t *p, uint_32 len)
{
    ...
    for (s = 0; s < 2; s++)
    {
        ...
        for (i = 0; i < 4; i++, dflag >>= 2)
        {
            ...
            /* ------------------------------------------------------------ */
            if ((dflag&0x14) != 0x10 && (dflag&0x0F) > 0x01) value |= 1u << i;
...
uint_32 pad_tk_keyboard(periph_t *p, uint_32 len)
{
    ...
    if (pad->io[0] == 1)
    {
        ...
        /* ---------------------------------------------------------------- */
        if ((pad->side[0] & (1u << 6)) == 0 && (pad->fake_shift & 2) != 0)
        {
            value |= 128;
            pad->fake_shift = 1;
        }
    ...
    else if (pad->io[1] == 1)
    {
        ...
        /* ---------------------------------------------------------------- */
        if ((pad->side[1] & (1u << 7)) == 0 && (pad->fake_shift & 2) != 0)
        {
            value |= 64;
            pad->fake_shift = 1;
        }

...
{
    const uint_32 addr    = jlp->ram[0x25],
                  row     = jlp->ram[0x26],
                  min_row = jlp->sg_start,
                  max_row = jlp->sg_end;
...
LOCAL void jlp_sg_ram_to_flash(jlp_t *jlp)
{
    uint_32 addr = jlp->ram[0x25] - 0x8000,
            row  = (jlp->ram[0x26] - jlp->ram[0x23]) * 192;
    int i, a;
...
LOCAL void jlp_sg_flash_to_ram(jlp_t *jlp)
{
    ...
    for (i = a = 0; i < 192; i += 2, a++)
    {
        uint_16 lo = jlp->sg_img[row + i + 0];
        uint_16 hi = jlp->sg_img[row + i + 1];
...
{
    uint_32 row = ((jlp->ram[0x26] - jlp->ram[0x23]) & -8) * 192;
...
)
{
    ...
    if (jlp->sg_bytes > 0)
    {
        if (!(jlp->sg_img = CALLOC(uint_8, jlp->sg_bytes)))
        {
            jzp_printf("JLP: out of memory\n");
            return -1;
        }

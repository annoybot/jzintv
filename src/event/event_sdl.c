...
struct evt_pvt_t
{
    uint_64 half_combos;            /* combos that might be forming         */
    uint_64 full_combos;            /* combos actively asserted             */
    uint_64 allocated_combos;       /* combos that are currently defined    */

    uint_32 combo_event_num[64];    /* event numbers associated w/ combos   */
    uint_64 combo_event_bit[64];    /* event bits associated w/ combos      */
    int     combo_event_cnt;        /* total # of unique events in combos   */

    uint_32 combo_pairs[64][2];     /* event #s assoc'd with each combo     */
...
LOCAL uint_32 event_tick(periph_p p, uint_32 len);
...
int event_init(event_t *event, int enable_mouse)
{
    ...
    event->periph.min_tick  = 3579545 / 4000;   /* 1000Hz */
    event->periph.max_tick  = 3579545 / 800;    /*  200Hz */
...
/* ======================================================================== */
LOCAL void event_dtor(periph_p p)
{
...
/* ======================================================================== */
LOCAL int event_queue_has_room(int count)
{
    return ev_q_wr - ev_q_ev_rd + count < EV_Q_LEN;
}
...
LOCAL uint_32 event_tick(periph_p p, uint_32 len)
{
	...
	getWiiJoyEvents();
#endif
    while (event_queue_has_room(4) && SDL_PollEvent(&ev))
    {
        switch (ev.type)
        {
            /* ------------------------------------------------------------ */
            ...
            case SDL_JOYBUTTONUP:
            {
                ...
                assert(!may_combo || ex_num == EVENT_IGNORE);
                if (may_combo)
                {
                    if (ev_updn == DOWN && event_chk_combo_press(event, ev_num))
                        goto leave;

                    if (ev_updn == UP && event_chk_combo_release(event, ev_num))
                        goto leave;
                } else
                {
...
)
{
    ...
    uint_64     event_bit = 1ull << combo_num;
    ...
    for (idx = 0; idx < pvt->combo_event_cnt; idx++)
        if (pvt->combo_event_num[idx] == (uint_32)num)
            break;

    if (idx == pvt->combo_event_cnt)
    {
        pvt->combo_event_cnt++;
        pvt->combo_event_num[idx] = num;
        pvt->in_combo[num >> 5] |= 1u << (num & 31);
    }

    if (pvt->combo_event_bit[idx] & event_bit)
    {
        fprintf(stderr, "event:  Must use two distinct events in a combo\n");
        return -1;
    }

    pvt->combo_event_bit[idx] |= event_bit;
...
{
    uint_32 event_bit = 1u << combo_num;
...
LOCAL uint_32 get_trailing_1(uint_64 v)
{
    const uint_32 lo = (uint_32)v;
    const uint_32 hi = (uint_32)(v >> 32);
    const uint_32 lbit = lo & -lo;
    const uint_32 hbit = hi & -hi;
    return 
        lo ? mult_DeBruijn_bit_pos[((uint_32)(lbit * 0x077CB531U)) >> 27]
           : mult_DeBruijn_bit_pos[((uint_32)(hbit * 0x077CB531U)) >> 27] + 32;
}
...
LOCAL int event_chk_combo_press(event_t *event, uint_32 ev_num)
{
    evt_pvt_t *pvt = event->pvt;
    int i;
    int ev_num_i = (ev_num >> 5), ev_num_m = 1u << (ev_num & 31);
    uint_64 new_combos, this_combos, prev_half_combos;
    int cork = 0;
    ...
    for (i = 0; i < pvt->combo_event_cnt; i++)
        if (pvt->combo_event_num[i] == ev_num)
            break;

    assert(i != pvt->combo_event_cnt);

    this_combos = pvt->combo_event_bit[i];
...
LOCAL int event_chk_combo_release(event_t *event, uint_32 ev_num)
{
    ...
    int i;
    int ev_num_i = (ev_num >> 5), ev_num_m = 1u << (ev_num & 31);
    uint_64 dying_combos, this_combos;
    int cork = 0;
        ...
        for (i = 0; i < 2; i++)
            if (pvt->combo_pairs[combo][i] != ev_num)
            {
                ...
                for (j = 0; j < pvt->combo_event_cnt; j++)
                    if (pvt->combo_event_num[j] == pvt->combo_pairs[combo][i])
                    {
                        if (!(pvt->full_combos & pvt->combo_event_bit[j]))

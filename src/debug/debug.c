...
/*                          stack.                                          */
/* ======================================================================== */
LOCAL uint_16 debug_infer_stack(cp1600_t *const cp)
{
    uint_32 val;
...
/* ======================================================================== */
LOCAL void debug_print_breakpt_cb(void *opaque, uint_32 addr)
{
...
LOCAL void debug_list_watches(const debug_t *const debug, 
                              const uint_32 *const bmap)
{
    const int fmt = debug->symb_addr_format;
    const char *symb = NULL;
    uint_32 addr_lo = 0, addr_hi = 0;
    int in_range = 0;
    uint_32 addr;

    for (addr = 0; addr <= 0x10000; addr++)
    {
        const uint_32 addr_idx = addr >> 5;
        const uint_32 addr_bit = (1u) << (addr & 0x1F);
...
uint_32 debug_rd(periph_t *p, periph_t *r, uint_32 a, uint_32 d)
{
    ...
    {
        jzp_printf(" RD a=%s d=%.4X %-16s (PC = %s) t=%" LL_FMT "u\n",
...
void    debug_wr(periph_t *p, periph_t *r, uint_32 a, uint_32 d)
{
    ...
    {
        jzp_printf(" WR a=%s d=%.4X %-16s (PC = %s) t=%" LL_FMT "u\n",
                debug_format_addr(debug, a, addrbuf, sizeof(addrbuf)),
                d, r==(periph_p)p->bus ? r->req->name:r->name,
                debug_format_addr(debug, cp->oldpc, pcbuf, sizeof(pcbuf)),
                cp->periph.now);
...
uint_32 debug_tk(periph_t *p, uint_32 len)
{
    ...
    int     req_ack, wind = 0, show_once = 0;
...
next_cmd:
        ...
        switch (cmd)
        {
            ...
            case 43:
            {
                ...
                const uint_32 sf = debug->stic->debug_flags;

...
LOCAL uint_8    *lzo_wrk = NULL, *lzo_inp = NULL, *lzo_out = NULL;
...
LOCAL void lzo_buf_alloc( long at_least )
{
    ...
    if (!lzo_wrk)
    {
        if (! (lzo_wrk = CALLOC(uint_8, LZO1X_MEM_COMPRESS)) ) goto oom;
    }
    ...
    if (needed)
    {
        ...
        lzo_inp = REALLOC(lzo_inp, uint_8, lzo_enc_buf_sz);
        lzo_out = REALLOC(lzo_out, uint_8, lzo_enc_buf_sz * 1088 / 1024);
        needed  = 0;
...
LOCAL void push_lzoe_info( const char *fname, long orig_len, long comp_len,
                           const uint_8 *cdata )
{
    uint_8 *buf = CALLOC( uint_8, comp_len );
...
LOCAL void write_csource( void )
{
    ...
    {
        fprintf( fc, "static const uint_8 cdata_%04d[] =\n{\n", i );

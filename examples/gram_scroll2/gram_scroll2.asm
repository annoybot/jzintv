;;==========================================================================;;
;; Joe Zbiciak's GRAM-based Scrolling Demo #2:  Large Squares               ;;
...
;*      as1600 -o gram_scroll2.bin -l gram_scroll2.lst gram_scroll2.asm     *;
;*                                                                          *;
;*  TO BUILD IN ROM FORMAT:                                                 *;
;*      as1600 -o gram_scroll2.rom -l gram_scroll2.lst gram_scroll2.asm     *;
;* ======================================================================== *;
    CFGVAR  "name" = "SDK-1600 GRAM-Based Scrolling Demo #2"
    CFGVAR  "short_name" = "GRAM Scroll 2"
    CFGVAR  "author" = "Joe Zbiciak"
    CFGVAR  "year" = 2002
    CFGVAR  "license" = "GPLv2+"
    CFGVAR  "description" = "Simulates screen scrolling via GRAM sequencing."
...
;; ======================================================================== ;;
;;  FILL_QUAD -- Writes to 1 corner of a 4x4 tile of cards.                 ;;
;; ======================================================================== ;;
FILL_QUAD   PROC
            MVII    #6,     R3
@@l2:       MVII    #10,    R2
...
;; ======================================================================== ;;
;;  SHIFT_DAT                                                               ;;
;;  This array just contains 1 SHL x, for 0 <= x <= 15.                     ;;
;; ======================================================================== ;;
SHIFT_DAT   PROC
            DECLE   0000000000000001b
            DECLE   0000000000000010b
            DECLE   0000000000000100b
            DECLE   0000000000001000b
            DECLE   0000000000010000b
            DECLE   0000000000100000b
            DECLE   0000000001000000b
            DECLE   0000000010000000b
            DECLE   0000000100000000b
            DECLE   0000001000000000b
            DECLE   0000010000000000b
            DECLE   0000100000000000b
            DECLE   0001000000000000b
            DECLE   0010000000000000b
            DECLE   0100000000000000b
            DECLE   1000000000000000b
            ENDP
...
@@oky:
            ...
            ;  We look at the 4 LSBs of the integer to determine the H and  ;
            ;  V offset.                                                    ;
            ; ============================================================= ;
            SWAP    R1,     1
            ANDI    #15,    R1      ; Horizontal offset 0..15
            SWAP    R2,     1
            ANDI    #15,    R2      ; Vertical offset 0..15

            ; ============================================================= ;
            ;  Generate pictures in GRAM that reflect our new offsets.      ;
            ; ============================================================= ;
            ADDI    #SHIFT_DAT, R1  ; Get shifted pixel for column
            MVI@    R1,     R0      ; 

            MVII    #$FF,   R1      ; Horizontal bar

            MVII    #$3810, R4      ; Point to GRAM cards #2 and #3.
            MVO@    R0,     R4      ; \                             
            MVO@    R0,     R4      ;  |
            MVO@    R0,     R4      ;  |                            
            MVO@    R0,     R4      ;  |
            ...
            MVO@    R0,     R4      ;  |
            MVO@    R0,     R4      ;  |                            
            MVO@    R0,     R4      ;  |-- Draw right 8 pixels of column
            NOP                     ;  |
            MVO@    R0,     R4      ;  |                            
            MVO@    R0,     R4      ;  |
            MVO@    R0,     R4      ;  |                            
            ...
            SWAP    R0              ; prepare to draw left 8 columns

            MVII    #$3800, R4      ; Point to GRAM cards #0 and #1.
            MVO@    R0,     R4      ; \                             
            MVO@    R0,     R4      ;  |
            MVO@    R0,     R4      ;  |                            
            MVO@    R0,     R4      ;  |
            ...
            MVO@    R0,     R4      ;  |
            MVO@    R0,     R4      ;  |                            
            MVO@    R0,     R4      ;  |-- Draw left 8 pixels of column
            NOP                     ;  |
            MVO@    R0,     R4      ;  |                            
            MVO@    R0,     R4      ;  |
            MVO@    R0,     R4      ;  |                            

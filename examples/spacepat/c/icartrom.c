...
{ 
    ...
    the_icart->extra = NULL;
    the_icart->extra_len = 0;

    if (decoded < len)
    {
        the_icart->extra = malloc(len - decoded);
        if (the_icart->extra)
        {
            memcpy((void*)the_icart->extra, (void*)(rom_img + decoded),
                    len - decoded);
            the_icart->extra_len = len - decoded;
        }
    }
...
uint_32 icartrom_writefile(char *fname, icartrom_t *the_icart, ictype_t type) 
{
    ...
    if (the_icart->extra && the_icart->extra_len > 0)
    {
        fwrite(the_icart->extra, 1, the_icart->extra_len, fr);
        size += the_icart->extra_len;
    }

...
void parse_profile(FILE *f)
{
    uint_32 c, a;
    int in_profile = 0;
    char instr[16], arg1[16], arg2[16], *s1, *s2;
    int a1, a2, r1, r2;
    int line_no = 0;
...
//printf("(B) %.8X / %s\n", a, addr[a].symb);
                ...
                if (strstr(buf, "PROC"))
                {
                    ...
                    cur_proc = strdup(lbl);
                    if (addr[a].symb) { free(addr[a].symb); addr[a].symb=NULL;}
//printf("(B) PROC: %s\n", cur_proc);
                }
...
//printf("(C) %.8X / %s\n", a, addr[a].symb);
        ...
        if (isspace(buf[0]) && strstr(buf, "ENDP"))
        {
            ...
            cur_proc = NULL;
            if ((addr[pa].flags & FL_PROC) == 0)
                addr[pa].flags |= FL_ENDP;
            else
                addr[pa].flags &= ~FL_PROC;
//printf("ENDP at %.4X, a = %.4X\n", pa, a);
        }
    }
...
void scan_procs(void)
{
    ...
    for (a = 0; a < 0x10000; a++)
    {
        ...
        if (((addr[a].flags & FL_ENDP) == FL_ENDP) && cur_proc)
        {
            if (procs >= MAXPROC)
            {
            ...
            {
//printf("closing cur_proc = %s at %.4X\n", cur_proc, a);
...
void print_loop_breakdown(void)
{
    ...
    printf("Loop breakdown for each function over 1%%\n\n");
